...
#
# From: https://github.com/conan-io/conan-center-index/blob/bad5c95b810e859c1c31553b92584246fe436d69/recipes/harfbuzz/all/conanfile.py
...
class HarfbuzzConan(ConanFile):
    ...
    default_options = {
        ...
        "with_freetype": True,
        "with_icu": False,
        "with_glib": False, # ASWF: complicated dependency
        "with_gdi": True,
        "with_uniscribe": True,
    ...
    def validate(self):
        # ASWF: annoying to check with a system wrapper
        ...
        # ASWF: test for Windows first
        if is_msvc_static_runtime(self) and self.options.with_glib and self.dependencies["glib"].options.shared:
            raise ConanInvalidConfiguration(
    ...
    def build_requirements(self):
        ...
        # ASWF: this makes trouble for dependencies
        # if self.options.with_glib:
    ...
    def package(self):
        copy(self, "COPYING", self.source_folder, os.path.join(self.package_folder, "licenses", self.name)) # ASWF: licenses in per package dir
        meson = Meson(self)
        meson.install()
