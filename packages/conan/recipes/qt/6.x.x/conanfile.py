...
#
# From: https://github.com/conan-io/conan-center-index/blob/1ce15d41e0301e69706f20bf3d6d942221d8baae/recipes/qt/6.x.x/conanfile.py
...
class QtConan(ConanFile):
    ...
    default_options = {
        ...
        "openssl": True,
        "with_pcre2": True,
        "with_glib": True, # ASWF: for OpenRV
        ...
        "with_sqlite3": True,
        "with_mysql": False,
        "with_pq": False,   # ASWF: skip database support
        "with_odbc": False, # ASWF: skip database support
        "with_zstd": True,
        "with_brotli": True,
    ...
    def configure(self):
        ...
        # ASWF: Define the submodules we need for ASWF build and PySide in particular
    ...
    def requirements(self):
        ...
        if self.options.get_safe("with_freetype", False) and not self.options.multiconfiguration:
            self.requires("freetype/[>=2.13 <3]", transitive_libs=True) # ASWF: don't pick up system freetype
        if self.options.get_safe("with_fontconfig", False):
            self.requires("fontconfig/2.15.0")
        if self.options.get_safe("with_icu", False):
            self.requires("icu/74.2")
        if self.options.get_safe("with_harfbuzz", False) and not self.options.multiconfiguration:
            self.requires("harfbuzz/[>=8.3.0]", transitive_libs=True) # ASWF: clients don't link otherwise
        if self.options.get_safe("with_libjpeg", False) and not self.options.multiconfiguration:
            if self.options.with_libjpeg == "libjpeg-turbo":
                self.requires("libjpeg-turbo/[~3]", transitive_libs=True) # ASWF: clients don't link otherwise
            else:
        ...
        if self.options.get_safe("with_md4c", False):
            self.requires("md4c/[>=0.4.8 <1]", transitive_libs=True) # stable API since 0.3x as per md4c wiki # ASWF otherwise does not link
    ...
    def generate(self):
        ...
        if not self.options.openssl:
            ...
            # ASWF: runtime breaks CMake config
        ...
        # be initialised before INSTALL_ARCHDATADIR is consumed.
        tc.cache_variables["INSTALL_ARCHDATADIR"] = "lib/qt6"
        tc.cache_variables["INSTALL_PLUGINSDIR"] = "lib/qt6/plugins"
        tc.cache_variables["INSTALL_QMLDIR"] = "lib/qt6/qml"
        tc.cache_variables["INSTALL_MKSPECSDIR"] = "lib/qt6/mkspecs"
        # ASWF: relocate arch-independent data (doc, phrasebooks, translations) out of package root.
        # Same reasoning: set derived variables explicitly alongside the root.
        tc.cache_variables["INSTALL_DATADIR"] = "share/qt6"
        tc.cache_variables["INSTALL_DOCDIR"] = "share/qt6/doc"
        tc.cache_variables["INSTALL_PHRASEBOOKSDIR"] = "share/qt6/phrasebooks"
        tc.cache_variables["INSTALL_TRANSLATIONSDIR"] = "share/qt6/translations"
        # ASWF: internal helper executables to libexec/qt6 (namespaced, FHS-correct for /usr/local install)
        tc.cache_variables["INSTALL_LIBEXECDIR"] = "libexec/qt6"
    ...
    def package(self):
        ...
        cmake = CMake(self)
        cmake.install()
        # ASWF: license files in per package dirs
        ...
        for module in self._get_module_tree:
            if not getattr(self.options, module):
                rmdir(self, os.path.join(self.package_folder, "licenses", self.name, module)) # ASWF: license files in per package dirs
        ...
        # ASWF: INSTALL_LIBEXECDIR=libexec/qt6
        os.remove(os.path.join(self.package_folder, "libexec" if self.settings.os != "Windows" else "bin", "qt6", "qt-cmake-private-install.cmake"))
        ...
        for target in targets:
            ...
            for path_ in [f"bin/{target}{extension}",
                          ...
                          f"libexec/qt6/{target}{extension}"]:  # ASWF: INSTALL_LIBEXECDIR=libexec/qt6
    ...
    def package_info(self):
        ...
        # ASWF: plugins relocated to lib/qt6/plugins (INSTALL_ARCHDATADIR=lib/qt6)
        self.runenv_info.define("QT_PLUGIN_PATH", os.path.join(self.package_folder, "lib", "qt6", "plugins"))
        self.buildenv_info.define("QT_PLUGIN_PATH", os.path.join(self.package_folder, "lib", "qt6", "plugins"))
        ...
        def _create_plugin(pluginname, libname, plugintype, requires):
            ...
            self.cpp_info.components[componentname].libdirs = [os.path.join("lib", "qt6", "plugins", plugintype)]  # ASWF: INSTALL_ARCHDATADIR=lib/qt6
        ...
        pkg_config_vars = [
            "bindir=${prefix}/bin",
            "libexecdir=${prefix}/libexec/qt6",  # ASWF: INSTALL_LIBEXECDIR=libexec/qt6
            "exec_prefix=${prefix}",
        ]
        ...
        self.cpp_info.components["qtPlatform"].set_property("cmake_target_name", "Qt6::Platform")
        self.cpp_info.components["qtPlatform"].includedirs = [os.path.join("lib", "qt6", "mkspecs", self._xplatform())]  # ASWF: INSTALL_ARCHDATADIR=lib/qt6
        if self.options.with_dbus:
        ...
        self.conf_info.define("user.qt:tools_directory", os.path.join(self.package_folder, "bin" if self.settings.os == "Windows" else os.path.join("libexec", "qt6")))  # ASWF: INSTALL_LIBEXECDIR=libexec/qt6
